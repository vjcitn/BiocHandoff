...
intro <- tryCatch(
    {
        description <- packageDescription("BiocHandoff")
...
This is the landing page of the `BiocBook` entitled "BiocHandoff".

This book introduces the reader to tasks required as staff members leave the organization.

We prefer a computable book to a static text or spreadsheet so that computations
can be included and checked.

# Docker image {-}

A `Docker` image built from this repository is available here: 

👉 [ghcr.io/vjcitn/biochandoff](https://ghcr.io/vjcitn/biochandoff) 🐳
...
#| eval: false
docker run -it ghcr.io/vjcitn/biochandoff:devel R
```
...
docker run \
    ...
    -e PASSWORD=OHCA \
    -p 8787:8787 \
    ghcr.io/vjcitn/biochandoff:devel
```
...
#| eval: false
docker run -it ghcr.io/vjcitn/biochandoff:devel bash
